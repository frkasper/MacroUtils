...
public class Demo11_Directed_Meshing extends StarMacro {
    ...
    private void post() {
        ...
        ud.scene = mu.add.scene.scalar(ud.namedObjects2,
                mu.get.objects.fieldFunction("Region Index", true), ud.unit_Dimensionless, false);
    ...
    private void pre() {
        mu.io.read.cameraView("Scalar|2.579937e-02,2.282902e-02,-8.461343e-03"
                + "|2.110428e-01,3.145835e-01,-1.134373e-01|-6.846060e-01,5.891221e-01,4.292433e-01"
                + "|7.413223e-02|1", true);
        mu.io.read.cameraView("Mesh|4.807398e-02,4.265680e-02,3.831531e-02"
                + "|3.986206e-01,3.932035e-01,3.888620e-01|0.000000e+00,1.000000e+00,0.000000e+00"
                + "|1.646111e-01|1", true);
        //--
        ...
        for (int i = 0; i < coords.length; i++) {
            ...
            for (int j = 0; j < coords.length; j++) {
                ...
                ud.cadPrt2 = mu.add.geometry.cylinder3DCAD(r, 2 * y, origin,
                ...
                mu.add.meshOperation.directedMeshing_Pipe(ud.partSrf1, ud.partSrf2,
                        10, 5, 40, 0.7, ud.csys);
                n++;
            }
        }
        //--
        ud.cadPrt = mu.add.geometry.block3DCAD(new double[]{ -x, -y, -z },
                new double[]{ x, y, z }, ud.unit_mm);
        ...
        ud.mshCtrl = mu.add.meshOperation.surfaceControl((AutoMeshOperation) ud.mshOp,
