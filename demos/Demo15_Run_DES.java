...
public class Demo15_Run_DES extends StarMacro {
    ...
    double mu_g = 1.855e-05;              //-- Viscosity.
    double rho0_g = 1.204;                //-- Reference Density.
    ...
    void initMacro() {
        ...
        ud.vv1 = mu.io.read.cameraView("cam1|5.530468e-02,-1.190976e-02,-7.199782e-02|4.282329e-01,2.739476e-01,5.776558e-01|-1.501948e-01,9.338231e-01,-3.246781e-01|8.359997e-02|1", true);
        ud.vv2 = mu.io.read.cameraView("cam2|1.841383e-02,2.223453e-02,-3.292230e-05|1.841383e-02,2.223453e-02,7.419417e-01|0.000000e+00,1.000000e+00,0.000000e+00|4.180352e-02|1", true);
    ...
    void setupPreRANS() {
        ud.cadPrt = mu.add.geometry.block3DCAD(new double[]{-L, -V, -L / 2}, new double[]{0, L, L / 2}, ud.unit_mm);
        ...
        ud.cadPrt = mu.add.geometry.block3DCAD(new double[]{0, 0, -L / 2}, new double[]{H, L, L / 2}, ud.unit_mm);
        ...
        ud.bdry.getConditions().get(KeTurbSpecOption.class).setSelected(KeTurbSpecOption.Type.INTENSITY_LENGTH_SCALE);
        ...
        mu.set.boundary.combine(mu.get.boundaries.allByREGEX(".*\\...", false)).setPresentationName(ud.bcWalls);
        //--
    ...
    void setupPostRANS() {
        ...
        mu.add.solver.stoppingCriteria(ud.repMon, StaticDeclarations.StopCriteria.ASYMPTOTIC, 0.01, 50);
    ...
    void setupPostDES() {
        ...
        //--
        mu.set.object.updateEvent(mu.get.monitors.byREGEX("Pressure Drop", true), ud.updEvent1, true);
        ...
        ud.ff2 = mu.add.tools.fieldFunction("Courant Number on LES Portion", "$CourantNumber * (1 - $SaTurbDesFd)",
                ud.dimDimensionless, FieldFunctionTypeOption.Type.SCALAR);
        ...
        mu.add.report.maximum(ud.point, "P1", mu.get.objects.fieldFunction(StaticDeclarations.Vars.P), ud.unit_Pa, true);
        //--
        //-- A few more Scenes
        ud.namedObjects2.clear();
        ud.namedObjects2.add(mu.add.derivedPart.isosurface(ud.namedObjects, ud.ff1, isoVal, ud.unit_Dimensionless));
        Scene scnStructures = mu.add.scene.scalar(ud.namedObjects2,
                mu.get.objects.fieldFunction(StaticDeclarations.Vars.TVR.getVar(), true), ud.defUnitPress, true);
        ...
        fixScalarDisplayer(scnStructures, false, tvrRange, 0, "%.0f",
                mu.get.objects.colormap(StaticDeclarations.Colormaps.LAND_ELEVATION), ud.dv0, ud.vv1);
        ud.namedObjects2.clear();
        ...
        Scene scnYPlus = mu.add.scene.scalar(ud.namedObjects2, ud.ff2, ud.unit_Dimensionless, false);
        ...
        fixScalarDisplayer(scnYPlus, false, new double[]{0, tvrRange[1] - 1}, 0, "%.1f",
                mu.get.objects.colormap(StaticDeclarations.Colormaps.BLUE_RED_BALANCED), ud.dv0, ud.vv1);
        //--
    ...
    void fixScalarDisplayer(Scene scn, boolean msh, double[] range, double w,
        ...
        ScalarDisplayer sd = (ScalarDisplayer) mu.get.scenes.displayerByREGEX(scn, "Scalar.*", true);
