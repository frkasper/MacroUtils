...
public class Demo5_Lego_Kart_Wind_Tunnel extends MacroUtils {
  ...
  void prep1_importGeometry() {
    ...
    defCamView = createCamView(camFocalPoint, camPosition, camViewUp, 0.07317047312477107, camName);
    ...
    camView2 = createCamView(camFocalPoint, camPosition, camViewUp, 0.07028763658591237, camName + "2");
    camView2.setPresentationName(camName + "2");
    importCADPart("LegoKart.x_b", 3, false);
  ...
  void prep3_BCsAndMesh() {
    ...
    for (Boundary bdry : getAllBoundaries()) {
        if (bdry.getPresentationName().startsWith(tunnelName)) { 
            ...
            setMeshBoundaryPrismSizes(bdry, 3, 1.2, 100);
        }
    }
  ...
  void prep4_setPost() {
    ...
    scene = createScene_Scalar(vecObj2, varP, defUnitPress, true);
    ...
    createStoppingCriteria(repMon, 0.25, 100, false);
    createReportForce(getCarBoundaries(), "F_y Car", new double[] {0, -1, 0});
    createStoppingCriteria(repMon, 0.0025, 100, false);
